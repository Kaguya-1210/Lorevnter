...
### ✅ Phase 3：管线整合与稳定性（已完成）

| 模块 | 状态 | 说明 |
|------|:----:|------|
| 审核→写入闭环 | ✅ | AI 分析→审核弹窗→确认后 uid 精确写回世界书 |
| 写回 uid 化 | ✅ | 条目按 uid 精确定位，消除同名误写风险 |
| 上下文语义分层 | ✅ | 历史→`<history_context>` + 最新→`<latest_context>` |
| CoT 思考链 | ✅ | AI 先在 `<lore_think>` 中逐条分析再输出 JSON |
| 约束 characterId 隔离 | ✅ | 局约束按角色隔离，不跨角色混用 |
| 条目过滤作用域对齐 | ✅ | UI 与管线使用相同的 `getActiveWorldbookNames()` |
| 模糊词精确化策略 | ✅ | AI 主动交叉比对条目，将模糊描述替换为具体信息 |
| 管线前刷新上下文 | ✅ | 执行前强制 `ctx.refresh()` 确保数据最新 |

### 🔜 Phase 4：功能补全（进行中）

| 待办 | 说明 |
|------|------|
| 人设写入功能 | 将 AI 分析结果写入角色人设卡 |
| 默认提示词优化 | 进一步调优内置 CoT 提示词结构 |
| UI 控件统一 | 统一输入框/按钮/弹窗的设计语言 |
| 移动端响应式增强 | 从"能用"到"真正适配" |
...
- 一次调用模式：筛选与更新合一
- CoT 思考链：AI 在 `<lore_think>` 标签中先逐条分析再输出 JSON
- 模糊词精确化：自动交叉比对条目，将模糊描述替换为具体信息
- 自动/手动触发两种方式
...
- **全部条目**：分析所有世界书条目（默认）
- **仅处理指定条目**（include）：只分析选中的条目
- **排除指定条目**（exclude）：分析除选中外的全部条目
- 搜索、全选、清空、反选，搜索结果全选只选搜索结果
- 过滤范围与管线分析范围完全对齐
